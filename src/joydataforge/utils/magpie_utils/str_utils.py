...
async def input_difficulty_rating(input):
    user_message = f'''
# Instruction 
...
async def input_classification(input):
    user_message = f'''
# Instruction
...
async def input_quality_rating(input):
    user_message = f'''
# Instruction
...
async def find_next_newline(string):
...
async def generate_variants(prefixes):
    variants = []
...
async def remove_prefix(string):
    ...
    prefixes = await generate_variants(prefixes)
...
async def instruction_post_process(instruction, model_path):
    if "gemma-2" in model_path.lower():
        # remove the prefix
        instruction = await remove_prefix(instruction)
        # find mcq problems
        is_mcq, end_pos = await find_mcq_end(instruction)
        assistant_markers = ["Answer:", "Answers:", "The answer is", "Correct answer", "The correct answer", "Answer is",
                             "Explanation:", "Here are some", "Solution Approach:", "Solution:"]
        ...
        assistant_pattern = r'(?:' + '|'.join(assistant_markers) + ')s?:'
        assistant_match = re.search(assistant_pattern, instruction)  # Exact match, no re.IGNORECASE
        logger.info(f"Assistant match: {assistant_match}")
        ...
        logger.info(f"Step match: {step_match}")
